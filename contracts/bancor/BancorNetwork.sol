...
contract BancorNetwork is IBancorNetwork, TokenHolder, ContractIds, FeatureIds {
    ...
    */
    function verifyTrustedSender(IERC20Token[] _path, uint256 _amount, uint256 _block, address _addr, uint8 _v, bytes32 _r, bytes32 _s) private returns(bool) {
        bytes32 hash = keccak256(abi.encodePacked(_block, tx.gasprice, _addr, msg.sender, _amount, _path));
    ...
    */
    function convertByPath(
        ...
        for (uint256 i = 1; i < pathLength; i += 2) {
            ...
            if(i == pathLength - 2){
              _amount = converter.change(_fromToken, toToken, _amount, _minReturn);
            } else {
              _amount = converter.change(_fromToken, toToken, _amount, 1);
            }
            _fromToken = toToken;
        }
