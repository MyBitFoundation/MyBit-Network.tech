...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // // @notice add this modifer to functions that you want multi-sig requirements for
  // // @dev function can only be called after at least n >= quorumLevel owners have agreed to call it
  // modifier hasConsensus(address _token, bytes4 _methodID, bytes32 _parameterHash){
  //   bytes32 proposalID = keccak256(abi.encodePacked(_token, address(this), _methodID, _parameterHash));
  //   uint256 approval = database.uintStorage(keccak256(abi.encodePacked("proposal.approval", _proposalID)));
  //   uint256 dissent = database.uintStorage(keccak256(abi.encodePacked("proposal.dissent", _proposalID)));
  //   uint256 voteCount = database.uintStorage(keccak256(abi.encodePacked("proposal.votecount", _proposalID)));
  //   require(approval.sub(dissent) >= database.uintStorage(keccak256(abi.encodePacked("token.quorum", _tokenAddress))));
  //   require(voteCount >= database.uintStorage(keccak256(abi.encodePacked("token.threshold", _tokenAddress))));
  //   _;
  // }
