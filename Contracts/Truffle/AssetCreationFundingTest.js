...
/*
   ...
   /*it('FundingHub payout to transition to stage 4', async () => {
     ...
     let investorPercentage = 100 - managerPercentage;
     let finalShares = (amountRaised / investorPercentage) * 100;
     let managerShares = finalShares * managerPercentage;


     assert.equal(await dbInstance.uintStorage(await hfInstance.stringBytesAddress('shares', assetID, assetCreator)), managerShares, 'managershares is correct');

     //1000000000000000000 / 95 * 100

     //console.log(Number(amountRaised));
   ...
   */
   ...
   it('Asset Recieves income', async () => {
     //await assetInstance.receiveIncome(assetID, 'firstPayment', {from:assetIncomePayer, value:web3.toWei(1,'ether')});
     //assert.equal(await dbInstance.uintStorage(await hfInstance.stringBytes('totalReceived', assetID)), web3.toWei(1,'ether'), 'totalReceived == 1 eth');
