...
contract('Ether Crowdsale', async(accounts) => {
  ...
  it('Start funding in the future', async() => {
    ...
    await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 100, bn(20).times(ETH), assetManagerFee, 10, platformToken.address, {from:assetManager});
  ...
  it('Asset already exists fail', async() => {
    ...
    try{
      await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 10, bn(20).times(ETH).times(1.03), assetManagerFee, 0, platformToken.address, {from:assetManager, gas:maxGas});
    } catch(e){
      err = e;
    }
  ...
  it('Start funding that does not reach goal', async() => {
    assetURI = 'No Goal';
    let block = await web3.eth.getBlock('latest');
    await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 2, bn(20).times(ETH), assetManagerFee, 0, platformToken.address, {from:assetManager, gas:maxGas});
    let logs = await events.getPastEvents('LogAsset', {filter: {messageID: web3.utils.sha3('Asset funding started'), manager: assetManager}, fromBlock: block.number});
    assetAddress = logs[0].args.asset;
    token = await Token.at(assetAddress);
  });
  ...
  it('Start funding', async() => {
    ...
    assetManagerFee = 0;
    let block = await web3.eth.getBlock('latest');
    await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 100, bn(2).times(ETH), assetManagerFee, 0, platformToken.address, {from:assetManager});
    let logs = await events.getPastEvents('LogAsset', {filter: {messageID: web3.utils.sha3('Asset funding started'), manager: assetManager}, fromBlock: block.number});
    assetAddress = logs[0].args.asset;
    token = await Token.at(assetAddress);
  });
  ...
  it('Fail funding with 100% manager fee', async() => {
    ...
    let err;
    try {
      await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 100, bn(2).times(ETH), assetManagerFee, 0, platformToken.address, {from:assetManager, gas:maxGas});
    }
    catch(e) {
      err = e;
    }
  ...
  it('Fail asset crowdsale with 0 amount to raise', async() => {
    ...
    let err;
    try{
      await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 100, 0, assetManagerFee, 0, platformToken.address, {from:assetManager, gas:maxGas});
    } catch(e){
      err = e;
    }
    assert.notEqual(err, undefined);
  ...
  it('Start funding with small amount to raise', async() => {
    ...
    assetManagerFee = 50;
    let block = await web3.eth.getBlock('latest');
    await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 100, 100, assetManagerFee, 0, platformToken.address, {from:assetManager, gas:maxGas});
    let logs = await events.getPastEvents('LogAsset', {filter: {messageID: web3.utils.sha3('Asset funding started'), manager: assetManager}, fromBlock: block.number});
    assetAddress = logs[0].args.asset;
    token = await Token.at(assetAddress);
  });
  ...
  it('Start funding then cancel', async() => {
    ...
    assetManagerFee = 0;
    let block = await web3.eth.getBlock('latest');
    await crowdsaleGen.createAssetOrderETH(assetURI, 'QmHash', 100, bn(10).times(ETH).toString(), assetManagerFee, 0, platformToken.address, {from:assetManager, gas:maxGas});
    let logs = await events.getPastEvents('LogAsset', {filter: {messageID: web3.utils.sha3('Asset funding started'), manager: assetManager}, fromBlock: block.number});
    assetAddress = logs[0].args.asset;
    token = await Token.at(assetAddress);
    //Fund
  ...
  it('Fail to create crowdsale, insufficient escrow (100%)', async() => {
    ...
    try{
      ...
      await crowdsaleGen.createAssetOrderETH('100% escrow', 'QmHash', 100, bn(1).times(ETH), assetManagerFee, bn(1).times(ETH), platformToken.address, {from:assetManager, gas:maxGas});
    } catch(e){
      err = e;
    }
    assert.notEqual(err, undefined);
  ...
  it('Fail to create crowdsale, insufficient escrow (35%)', async() => {
    ...
    try{
      ...
      await crowdsaleGen.createAssetOrderETH('35% escrow', 'QmHash', 100, bn(1).times(ETH), assetManagerFee, bn(0.35).times(ETH), platformToken.address, {from:assetManager, gas:maxGas});
    } catch(e){
      err = e;
    }
    assert.notEqual(err, undefined);
  ...
  it('Fail to create crowdsale, insufficient escrow (35%*3)', async() => {
    ...
    try{
      ...
      await crowdsaleGen.createAssetOrderETH('105% escrow', 'QmHash', 100, bn(1).times(ETH), assetManagerFee, bn(1.05).times(ETH), platformToken.address, {from:assetManager, gas:maxGas});
    } catch(e){
      err = e;
    }
    assert.notEqual(err, undefined);
