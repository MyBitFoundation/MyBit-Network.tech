...
/*
  ...
  */
  ...
  it('Load platform token', async() => {
    myb = await PlatformToken.at(addresses.MyBit.toString());
  });
  ...
  it('Load ERC20Burner', async() => {
    burner = await ERC20Burner.at(addresses.ERC20Burner.toString());
  });
  ...
  it('Give platform burning permission', async() => {
    for(var i=1; i<web3.eth.accounts.length; i++){
      await myb.approve(burner.address, tokenSupply, {from:web3.eth.accounts[i]});
